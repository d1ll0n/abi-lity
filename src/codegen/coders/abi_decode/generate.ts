...
export function buildExternalWrapper(
  ...
  const ctx: WrappedScope = WrappedSourceUnit.getWrapper(helper, externalFileName);
  const sourceUnit = helper.getSourceUnit(primaryFileName);
  ...
  addFunctionImports(ctx.sourceUnit, functions, aliases as any);
  ...
  ctx.sourceUnit.appendChild(contract);
