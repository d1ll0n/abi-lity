...
MemoryPointer constant ZeroSlot = MemoryPointer.wrap(0x60);
MemoryPointer constant ScratchPtr = MemoryPointer.wrap(0x00);
uint256 constant IdentityPrecompileAddress = 0x4;
uint256 constant OffsetOrLengthMask = 0xffffffff;
uint256 constant OneWord = 0x20;
uint256 constant OneWordShift = 0x05;
uint256 constant FreeMemoryPointerSlot = 0x40;
uint256 constant ThirtyOneBytes = 0x1f;
uint256 constant SixtyThreeBytes = 0x3f;
uint256 constant NinetyFiveBytes = 0x5f;
...
library MemoryPointerLib {
  function viewCopy(MemoryPointer src, MemoryPointer dst, uint256 size) private view {
    assembly {
  ...
  function pureCopy() private pure returns (function(MemoryPointer, MemoryPointer, uint256) internal pure _pureCopy) {
    function(MemoryPointer, MemoryPointer, uint256) internal view _viewCopy = viewCopy;
    assembly {
      _pureCopy := _viewCopy
    }
  }

  function copy(MemoryPointer src, MemoryPointer dst, uint256 size) internal pure {
    pureCopy()(src, dst, size);
  }
  ...
  function returnData(MemoryPointer mPtr, uint256 size) internal pure {
    assembly {
      return(mPtr, size)
    }
  }
