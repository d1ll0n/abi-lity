...
function return_tuple_bool_bool_uint256_uint256(bool value0, bool value1, uint256 value2, uint256 value3) pure {
  bytes memory returnData = abi.encode(value0, value1, value2, value3);
  assembly {
    return(add(returnData, 32), mload(returnData))
  }
}
function return_tuple_string_bytes32_address(string memory value0, bytes32 value1, address value2) pure {
  bytes memory returnData = abi.encode(value0, value1, value2);
  assembly {
    return(add(returnData, 32), mload(returnData))
  }
}
function return_string(string memory contractName) pure {
  bytes memory returnData = abi.encode(contractName);
  assembly {
    return(add(returnData, 32), mload(returnData))
  }
}